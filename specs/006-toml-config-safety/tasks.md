...
- [x] T005 Update `internal/config/template.go` to produce a documented `dbmask.toml`

---

## Phase 2: CLI & Engine Refactoring ✅

- [x] T006 Update `cmd/dbmask/root.go` to use `dbmask.toml` as default configuration path
- [x] T007 Refactor `cmd/dbmask/validate.go` to use the new TOML parser
- [x] T008 Update `cmd/dbmask/init.go` to support TOML initialization
- [x] T009 Refactor `cmd/dbmask/apply.go` to ensure `dsn` is read from config, and safety gates are non-interactive
- [x] T010 Refactor `cmd/dbmask/doctor.go` to verify TOML config and connection readiness

---

## Phase 3: Safety & In-Place Mutation ✅

- [x] T011 Enhance `masker.Engine` to strictly enforce `DryRun` (zero writes verified)
