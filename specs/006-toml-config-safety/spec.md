...
**Created**: 2026-02-07  
**Status**: Draft  
**Input**: User description: "SRS: dbmask — TOML Configuration, Autonomous & Safe Inline Masking VERSION: 6.0"
...
**Independent Test**: Can be tested by creating a `dbmask.toml` file, running `dbmask validate`, and ensuring it correctly identifies valid and invalid TOML configurations according to the defined schema.

**Acceptance Scenarios**:

1. **Given** a valid `dbmask.toml` with `[database]`, `[safety]`, and `[[tables]]` sections, **When** I run `dbmask validate`, **Then** the tool reports the configuration is valid.
2. **Given** a configuration file with an unknown attribute (e.g. `[unknown] section`), **When** I run `dbmask validate`, **Then** the tool fails with a clear validation error.
...
**Independent Test**: Run `dbmask apply --dry-run` against a test database and verify that while rows are fetched and values generated, zero `UPDATE` statements are committed.

**Acceptance Scenarios**:

1. **Given** a valid TOML configuration, **When** I run `dbmask apply --dry-run`, **Then** the tool connects to the DB, performs plan calculation, and generates masked values, but exits with a message indicating no changes were written.
...
**Independent Test**: Run `dbmask plan` and verify the output contains table names, column names, generator types, and a limited preview of data changes.

**Acceptance Scenarios**:

1. **Given** a configured table `users`, **When** I run `dbmask plan`, **Then** I see an estimate of row counts and a preview showing `email: secret@company.com -> user_123@example.org`.
...
- **Malformed DSN**: `dbmask doctor` and `dbmask apply` must fail fast with a clear error if the DSN format is unsupported or invalid.
...
- **FR-004**: System MUST provide `dbmask apply --dry-run` which performs zero database writes.
- **FR-005**: `dbmask plan` MUST show affected tables, columns, row estimates, and sample diffs (max 5 rows).
- **FR-006**: `dbmask apply` MUST run full validation, planning, and safety checks before mutation.
- **FR-007**: `dbmask doctor` MUST perform non-destructive pre-flight checks including dry-run capability verification.
...
- **SC-002**: `dbmask apply --dry-run` performs 0 commits to the database.
...
- **SC-005**: `dbmask doctor` correctly identifies a database not in the `allowed_db_names` list.