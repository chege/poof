...
- [x] T005 Refactor `internal/poof/engine.go` to depend exclusively on `db.DB`
...
- [x] T009 Update `cmd/poof/apply.go` with `--dry-run` flag and enhanced safety gates
- [x] T010 Update `cmd/poof/plan.go` to display table summaries and row estimates
- [x] T011 Update `cmd/poof/doctor.go` to verify DSN support and dry-run availability
