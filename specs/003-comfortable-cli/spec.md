...
**Independent Test**: Run `poof init`, verify `poof.toml` is created, and ensure `poof validate` passes on the generated file.

**Acceptance Scenarios**:

1. **Given** an empty directory, **When** I run `poof init`, **Then** a `poof.toml` file is created with valid syntax and commented examples.
...
- **Malformed TOML in Init**: `init` should never produce malformed TOML.
...
- **FR-002**: System MUST provide an `init` command that generates a documented `poof.toml` file.
- **FR-003**: System MUST provide a `validate` command that checks TOML syntax and schema (unknown fields/blocks).
...
- **Standard Config**: The opinionated TOML template generated by `init`.
